...
class LaneRobotLoss:
    """UFLD-v2-like loss for LaneRobot/LaneRobotV2 row-anchor lane detection.

    Supports either cls-only predictions [B, X+1, R, L] or a dict with:
      - cls: [B, X+1, R, L]
      - offset: [B, 1, R, L]

    Targets:
      batch["lane"] is integer class target [B, R, L], with X as no-lane.
      batch["lane_x"] is optional float target [B, R, L], with -1 for no-lane.
    """
    ...
    def __init__(self, model):
        ...
        self.lambda_ce = float(getattr(args, "lane_ce", 1.0)) if args is not None else 1.0
        self.lambda_loc = float(getattr(args, "lane_loc", 0.5)) if args is not None else 0.5
        self.lambda_exist = float(getattr(args, "lane_exist", 1.0)) if args is not None else 1.0
        self.lambda_smooth = float(getattr(args, "lane_smooth", 0.05)) if args is not None else 0.05
        self.lambda_curv = float(getattr(args, "lane_curv", 0.05)) if args is not None else 0.05
        ...
        if isinstance(raw_task_weights, str):
            raw_task_weights = [float(x.strip()) for x in raw_task_weights.split(",") if x.strip()]
    ...
    def _target_tensors(self, batch, device):
        import torch

        target = batch.get("lane", batch.get("lanes", None))
        if target is None:
            raise KeyError("Lane task requires batch['lane'] with shape [B, row_anchors, num_lanes].")
        target = target.to(device).long()
        if target.ndim == 2:
            target = target.unsqueeze(0)
        if target.shape[1:] == (self.num_lanes, self.row_anchors):
            target = target.transpose(1, 2).contiguous()
        if target.shape[1:] != (self.row_anchors, self.num_lanes):
            raise ValueError(f"Expected lane target [B,{self.row_anchors},{self.num_lanes}], got {tuple(target.shape)}.")
        target = target.clamp(min=-1, max=self.x_grids)
        target = torch.where(target < 0, torch.full_like(target, self.no_lane_idx), target)

        target_x = batch.get("lane_x", None)
        if target_x is None:
            target_x = target.float()
            target_x = torch.where(target == self.no_lane_idx, torch.full_like(target_x, -1.0), target_x)
        else:
            target_x = target_x.to(device).float()
            if target_x.ndim == 2:
                target_x = target_x.unsqueeze(0)
            if target_x.shape[1:] == (self.num_lanes, self.row_anchors):
                target_x = target_x.transpose(1, 2).contiguous()
    ...
    def _compute_single_task_loss(self, logits, offset, target, target_x, valid):
        """Compute the original LaneRobotV2 loss for one independent task only."""
        import torch
        import torch.nn.functional as F
        ...
        components = torch.stack((ce, loc, exist, smooth, curv, offset_loss))
        total = (
            self.lambda_ce * ce
            + self.lambda_loc * loc
            + self.lambda_exist * exist
    ...
    def _task_weight_tensor(self, device, dtype):
        """Return normalized per-task weights while keeping backward compatibility with single-task configs."""
        import torch

        raw = getattr(self, "task_weights", None)
        if raw is None:
            return torch.full((self.num_lanes,), 1.0 / self.num_lanes, device=device, dtype=dtype)
        weights = torch.as_tensor(raw, device=device, dtype=dtype).flatten()
        if weights.numel() != self.num_lanes:
            raise ValueError(
                f"lane_task_weights must contain {self.num_lanes} values, got {weights.numel()}: {raw}"
            )
        if (weights < 0).any() or not torch.isfinite(weights).all() or weights.sum() <= 0:
            raise ValueError(f"lane_task_weights must be finite, non-negative, and not all zero, got {raw}")
        return weights / weights.sum()

    def __call__(self, preds, batch):
        """Compute each fixed lane task independently, then combine the task losses by normalized weights."""
        import torch

        logits, offset = self._split_preds(preds)
        target, target_x, valid = self._target_tensors(batch, logits.device)

        expected_cls = (self.x_grids + 1, self.row_anchors, self.num_lanes)
        if tuple(logits.shape[1:]) != expected_cls:
            raise ValueError(f"Expected cls [B,{expected_cls[0]},{expected_cls[1]},{expected_cls[2]}], got {tuple(logits.shape)}")
        if offset is not None and tuple(offset.shape[1:]) != (1, self.row_anchors, self.num_lanes):
            raise ValueError(
                f"Expected offset [B,1,{self.row_anchors},{self.num_lanes}], got {tuple(offset.shape)}"
            )

        task_totals = []
        task_components = []
        for task_id in range(self.num_lanes):
            task_offset = offset[..., task_id : task_id + 1] if offset is not None else None
            total_i, components_i = self._compute_single_task_loss(
                logits=logits[..., task_id : task_id + 1],
                offset=task_offset,
                target=target[..., task_id : task_id + 1],
                target_x=target_x[..., task_id : task_id + 1],
                valid=valid[..., task_id : task_id + 1],
            )
            task_totals.append(total_i)
            task_components.append(components_i)

        task_totals = torch.stack(task_totals)
        task_components = torch.stack(task_components)  # [L, 6]
        weights = self._task_weight_tensor(logits.device, logits.dtype)
        loss = (task_totals * weights).sum()
        averaged_components = (task_components * weights[:, None]).sum(dim=0)

        # Exposed for debugging/monitoring without changing the existing six-item trainer contract.
        self.last_task_total_losses = task_totals.detach()
        self.last_task_loss_items = task_components.detach()
        return loss, averaged_components.detach()
