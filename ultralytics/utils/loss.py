...
class LaneRobotLoss:
    ...
    def __call__(self, preds, batch):
        ...
        if valid_pair.any():
            ...
            smooth = F.smooth_l1_loss(d1[valid_pair] / max(self.x_grids - 1, 1), torch.zeros_like(d1[valid_pair]))
        else:
        ...
        if valid_triplet.any():
            ...
            curv = F.smooth_l1_loss(d2[valid_triplet] / max(self.x_grids - 1, 1), torch.zeros_like(d2[valid_triplet]))
        else:
            curv = logits.sum() * 0.0

        loss_items = torch.stack((ce.detach(), loc.detach(), exist.detach(), smooth.detach(), curv.detach(), offset_loss.detach()))
        loss = (
            self.lambda_ce * ce
            + self.lambda_loc * loc
