...
class LaneRobot(nn.Module):
    """YOLOP model.34-compatible row-anchor lane head.

    Exact topology from the supplied ONNX lane_robot branch:
        Conv1x1 -> Flatten -> Linear -> ReLU -> Linear -> Reshape

    Output shape is [B, x_grids + 1, row_anchors, num_lanes]. The extra x class is the no-lane class.
    """
    ...
    def __init__(
        ...
        # Keep this as raw nn.Conv2d, not Conv+BN+act, to match ONNX /model.34/conv_1x1/Conv exactly.
        ...
        self.out_dim = (self.x_grids + 1) * self.row_anchors * self.num_lanes
        self.cls = nn.Sequential(
            nn.Linear(self.flatten_dim, self.hidden_dim),
    ...
    def forward(self, x):
        ...
        Args:
            x (Tensor): stride-32 feature, normally [B, C, 8, 10] for 256x320 input.
        """
        x = self.conv_1x1(x)
        if x.shape[-2:] != (self.feat_h, self.feat_w):
            # Preserve the FC topology while allowing another input size during experimentation.
            # For exact ONNX parity, train with imgsz=[256, 320] so this branch is not used.
            x = nn.functional.adaptive_avg_pool2d(x, (self.feat_h, self.feat_w))
        x = torch.flatten(x, 1)
        x = self.cls(x)
        return x.view(x.shape[0], self.x_grids + 1, self.row_anchors, self.num_lanes)


class LaneRobotV2(nn.Module):
    """UFLD-v2-like row-anchor lane head with classification and offset outputs.
    ...
    The class dimension includes the no-lane class at index x_grids.
    """
    ...
    def __init__(
        ...
        self.conv_1x1 = nn.Conv2d(c1, self.reduce_channels, kernel_size=1, stride=1, padding=0, bias=True)
        self.pool = nn.AdaptiveAvgPool2d((self.feat_h, self.feat_w))
        self.flatten_dim = self.reduce_channels * self.feat_h * self.feat_w
...
class SingleLaneRobotV2Branch(nn.Module):
    """One independent copy of the proven single-task LaneRobotV2 prediction branch.

    The layer topology intentionally matches ``LaneRobotV2(num_lanes=1)`` exactly:
    Conv1x1 -> AdaptiveAvgPool2d -> Flatten -> FC -> ReLU -> cls/offset outputs.
    """
    ...
    def __init__(
        ...
        self.cls_fc1 = nn.Linear(self.flatten_dim, self.hidden_dim)
        self.act = nn.ReLU(inplace=False)
        self.cls_fc2 = nn.Linear(self.hidden_dim, (self.x_grids + 1) * self.row_anchors)
        self.offset_fc = nn.Linear(self.hidden_dim, self.row_anchors)

    def forward(self, x):
        """Return one task's cls logits ``[B, X+1, R]`` and offsets ``[B, 1, R]``."""
        x = self.conv_1x1(x)
        x = self.pool(x)
        x = torch.flatten(x, 1)
        feat = self.act(self.cls_fc1(x))
        b = feat.shape[0]
        cls = self.cls_fc2(feat).view(b, self.x_grids + 1, self.row_anchors)
        offset = torch.tanh(self.offset_fc(feat)).view(b, 1, self.row_anchors) * 0.5
        return cls, offset


class LaneRobotV2Independent(nn.Module):
    """Shared-backbone lane head with one fully independent LaneRobotV2 branch per fixed task.

    Each task predicts its own ``[X+1, R]`` row-anchor distribution. The task dimension is
    only a stack of independent outputs; no softmax or competition is applied across tasks.

    Returns during training/inference:
        ``{"cls": [B, X+1, R, L], "offset": [B, 1, R, L]}``

    With ``export=True`` the two tensors are concatenated along dimension 1 and returned as
    ``[B, X+2, R, L]`` for compatibility with the existing deployment path.
    """
    ...
    def __init__(
        ...
        self.row_anchors = int(row_anchors)
        self.num_lanes = int(num_lanes)
        self.num_lane_tasks = self.num_lanes
        self.reduce_channels = int(reduce_channels)
        self.hidden_dim = int(hidden_dim)
        self.feat_h = int(feat_h)
        self.feat_w = int(feat_w)
        ...
        if self.x_grids < 1:
            raise ValueError(f"x_grids must be >= 1, got {self.x_grids}")
        if self.row_anchors < 1:
            raise ValueError(f"row_anchors must be >= 1, got {self.row_anchors}")
        if self.num_lanes < 1:
    ...
    def forward(self, x):
        ...
        for branch in self.task_branches:
            cls_i, offset_i = branch(x)
            cls_outputs.append(cls_i)
            offset_outputs.append(offset_i)

        cls = torch.stack(cls_outputs, dim=-1)
        offset = torch.stack(offset_outputs, dim=-1)
        if self.export:
            return torch.cat((cls, offset), dim=1)
        return {"cls": cls, "offset": offset}
